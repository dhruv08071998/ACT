...
- (IBAction)loginButtonPressed:(id)sender {
    UIImage* selectedImg=[UIImage imageNamed:@"checkbox_fill"];
        if (_btnCheckBox.currentImage == selectedImg )
        {
    [self hideKeyboard];
...
//        [self handleLoginAttempt:[delegate loginWithUsername:@"test1@gmail.com"
//                                                    password:@"871998dhruv"]];
    }
    [emailField resignFirstResponder];
    [passwordField resignFirstResponder];
        } else {
            NSError *err = [NSError errorWithDomain:@"some_domain"
                                               code:100
                                           userInfo:@{
                                                       NSLocalizedDescriptionKey:@"Please Accept Terms And Conditions"
                                           }];
            [self showAlertWithTitle:@"ACT" withError:err];
        }
}
...
- (IBAction)wantToChangeUserTypeTapped:(id)sender {
}
- (IBAction)btnTermsTapped:(id)sender {
}
...
-(void) handleLoginAttempt: (RXPromise *) promise {
    ...
    promise.thenOnMain(
        ...
        [user setMetaValue:[self getuserType] forKey:@"userType"];
                    return Nil;
                }, ^id(NSError * error) {
                    __typeof__(self) strongSelf = weakSelf;
                    if ([error.localizedDescription  isEqual: @"There is no user record corresponding to this identifier. The user may have been deleted."]){
                        [self registerButtonPressed:self->registerButton];
                    } else {
                    [strongSelf showAlertWithTitle:[NSBundle t:bErrorTitle] withError:error];
                    [strongSelf hideHUD];
                    }
