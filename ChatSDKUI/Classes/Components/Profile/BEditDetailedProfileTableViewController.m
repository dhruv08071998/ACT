...
- (void)viewDidLoad {
    ...
    if ([user.name rangeOfString:@"ChatSDK"].location == NSNotFound) {
...
-(void) save {
    bool flagEmail = [self validateEmailWithString:emailTextField.text];
    bool flagName = [self isAlphanumeric:nameTextField.text];
    if (!flagName) {
        [self alertWithTitle:@"ACT" withMessage: @"Please enter valid name"];
    } else if (!flagEmail) {
        [self alertWithTitle:@"ACT" withMessage: @"Please enter valid email"];
    } else {
        [self updateUserAndIndexes];
