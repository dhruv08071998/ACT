...
-(void) viewWillAppear:(BOOL)animated {
    ...
    if (!self.navigationItem.leftBarButtonItem && user.isMe && BChatSDK.ui.settingsSections.count > 0 && BChatSDK.ui.settingsViewController) {
        ...
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[NSBundle uiImageNamed:@"Group 97"] style:UIBarButtonItemStylePlain target:self action:@selector(settings)];
    }
...
-(void) viewDidAppear:(BOOL)animated {
    ...
    _btnStartChat.layer.cornerRadius = 10;
}
...
//    }];
    ...
    UIViewController *add =
               [storyboard instantiateViewControllerWithIdentifier:@"ChatThreadVC"];
    [add setModalPresentationStyle: UIModalPresentationFullScreen];
...
- (IBAction)closeChatTapped:(id)sender {
    [self dismissViewControllerAnimated:YES completion:nil];
//    var detailVC =  BDetailedProfileTableViewController()
