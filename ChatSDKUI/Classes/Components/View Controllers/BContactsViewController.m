...
-(instancetype) initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.tabBarItem.image = [NSBundle uiImageNamed: @"icn_30_contact"];
...
//
//    vc.view.frame = self.view.frame;  //Set a frame or constraints
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<PUserConnection> connection = _contacts[indexPath.row];
    [BChatSDK.thread createThreadWithUsers:@[connection.user] threadCreated:^(NSError * error, id<PThread> thread) {
        _chatUserName = thread.displayName;
        _chatThread = thread;
        [self getThreadAndName];
    }];
    ...
    UINavigationController *navBar = [[UINavigationController alloc]initWithRootViewController:profileView];
    navBar.modalPresentationStyle = UIModalPresentationFullScreen;
